    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            for (int x = 0; x < SIZE; x++)
            {
                ...
                {
                    grid[x, y] = new Tile();
                    grid[x, y].pos = new Coordinate() { X = x, Y = y };
                }
            }
        ...
        private static void FillWithOxygen()
        {
            ...
            Coordinate start = sensor;
            grid[start.X, start.Y].OxygenSpreadTo = true;
            ...
            while (true)
            {
                ...
                foreach (var item in filled)
                {
                    ...
                    // N
                    if (grid[item.pos.X - 1, item.pos.Y].Type != TileType.WALL &&
                        grid[item.pos.X - 1, item.pos.Y].OxygenSpreadTo == false &&
                        grid[item.pos.X - 1, item.pos.Y].OxygenSpreadedFrom == false
                    )
                    {
                        grid[item.pos.X - 1, item.pos.Y].OxygenSpreadTo = true;
                        grid[item.pos.X - 1, item.pos.Y].OxygenSpreadedFrom = false;
                    }
                    ...
                    // W
                    if (grid[item.pos.X, item.pos.Y - 1].Type != TileType.WALL &&
                        grid[item.pos.X, item.pos.Y - 1].OxygenSpreadTo == false &&
                        grid[item.pos.X, item.pos.Y - 1].OxygenSpreadedFrom == false
                    )
                    {
                        grid[item.pos.X, item.pos.Y - 1].OxygenSpreadTo = true;
                        grid[item.pos.X, item.pos.Y - 1].OxygenSpreadedFrom = false;
                    }
        ...
        private static List<Direction> GetVisitableDirectionsWithoutOxygen()
        {
            var ret = new List<Direction>();
            if (GetTileInDirection(Direction.N).Type != TileType.WALL)
            {
                ret.Add(Direction.N);
            }
            if (GetTileInDirection(Direction.S).Type != TileType.WALL)
            {
                ret.Add(Direction.S);
            }
            if (GetTileInDirection(Direction.W).Type != TileType.WALL)
            {
                ret.Add(Direction.W);
            }
            if (GetTileInDirection(Direction.E).Type != TileType.WALL)
            {
                ret.Add(Direction.E);
            }
...

                {
                    ...
                    for (int i = 0; i < SIZE; i++)
                    {
                        for (int j = 0; j < SIZE; j++)
                        {
                            ...
                            {
                                if (grid[i, j].OxygenSpreadTo == true)
                                {
                                    Console.Write("O");
                                }
                                else
